...
(function (root, factory) {
...
}(this, function(moment,noGlobal) {    
    'use strict';
    var document = window.document;
    var version = 0.1,
    strundefined = typeof undefined;
    var hms = window.hms || {}; // 定義namespace, 名稱固定為hms
    hms.util = hms.util || {}; // 定義namespace, 名稱util可自定為其他單字
    hms.util.Lightpick = hms.util.Lightpick || function() { // 定義物件名稱, 名稱fn可自定為其他單字
    }
    var defaults = {
        field: null,
        secondField: null,
        ...
        tooltipNights: false,
        todayButton:true,
        todayButtonTexts:['This Month','This Year'],
        locale: {
            buttons:{ prev: 'fa-angle-left', 
                      next: 'fa-angle-right',
                      close:'fa-times',
                      reset:'Reset',
                      apply:'Apply'
    ...
    var renderTopButtons = function(opts, viewMode)
    {
        var todayButton = viewMode=="months"?opts.todayButtonTexts[1]:opts.todayButtonTexts[0];
        return '<div class="lightpick__toolbar">'
            ...
            + (opts.todayButton ?'<button type="button" class="lightpick__today-action" data-view-mode="' + viewMode + '">'+todayButton+'</button>':'')
            + '<button type="button" class="lightpick__previous-action" data-view-mode="' + viewMode + '"><span class="lightpick__previous-action fa '+opts.locale.buttons.prev+'" data-view-mode="' + viewMode + '"></span></button>'
            + '<button type="button" class="lightpick__next-action" data-view-mode="' + viewMode + '"><span class="lightpick__next-action fa '+opts.locale.buttons.next+'" data-view-mode="' + viewMode + '"></span></button>'
            + (!opts.autoclose ? '<button type="button" class="lightpick__close-action"><span class="lightpick__close-action fa '+opts.locale.buttons.close+'"></span></button>'  : '')
            + '</div>';
    }

    var weekdayName = function(opts, day, short)
    {
        return new Date(1970, 0, day).toLocaleString(opts.lang, { weekday: short ? 'short' : 'long' })
        //return moment("1970/01/"+day, "YYYY/MM/DD").format("ddd");
    }

    var renderDay = function(opts, date, dummy, extraClass)
    {
    ...
    var renderCalendar = function(el, opts) 
    {
        var html = '',
            monthDate = moment(opts.calendar[0]),
            selDate = moment(opts.calendar[0]);
        ...
        for (var i = 0; i < opts.numberOfMonths; i++) {
            ...
            //+ '<b class="lightpick__month-title-accent">' + day.format("MMM") + '</b> '
            //純英文
            //+ day.format("MMM")+"  "
            //+ day.format('YYYY')  
        ...
        $(".lightpick__previous-action").addClass('disable_btn');
        if(opts.minDate){
            if(selDate.year() > opts.minDate.year()){
                $(".lightpick__previous-action").removeClass('disable_btn');
            }
            else if(selDate.year()==opts.minDate.year() && selDate.month()>(opts.minDate.month())){
                $(".lightpick__previous-action").removeClass('disable_btn');
            }            
        }
        else{
    ...
    var renderMonthsOfYear = function(el, opts, date)
    {
        if (date) {
        ...
        //var minDateMonth = (0 || opts.minDate.month()+1);
        var dataGotoMonth ,bgclr,activeEvent;
       
        for (var i = 1; i <= 12; i++) {
            if(opts.minDate){
                //dataGotoMonth = ym.format("YYYY") + '-' + i;
                //bgclr = "";
                dataGotoMonth = "";
                activeEvent = false;
                bgclr = "style='background-color:#ececec;color:grey;'";
                if(ym.format("YYYY")>opts.minDate.year()){
                    dataGotoMonth = ym.format("YYYY") + '-' + i;
                    bgclr = "";
                    activeEvent = true;
                }
                else if(ym.format("YYYY")==opts.minDate.year() && i>=(opts.minDate.month()+1)){
                    dataGotoMonth = ym.format("YYYY") + '-' + i;
                    bgclr = "";
                    activeEvent = true;
                }
            ...
            html += '<div class="lightpick__month-of-the-year" data-goto-month="'+dataGotoMonth+'" '+bgclr+'>' 
                 //原始程式
                 //+ '<div>' + moment(i, 'M').toDate().toLocaleString(opts.lang, { month: 'long' }) + '</div>'
                 //+ '<div data-goto-month="' + ym.format('YYYY') + '-' + i + '">' + moment(i, 'M').format("MMMM")+ '</div>'
                 //+ '<div data-goto-month="' + ym.format('YYYY') + '-' + i + '">' + ym.format('YYYY') + '</div>'
                 //純英文版
                 //+ '<div'+(activeEvent?' data-goto-month="' + ym.format('YYYY') + '-' + i + '"':'')+'>' + moment(i, 'M').format("MMMM")+ '</div>'
                 //+ '<div'+(activeEvent?' data-goto-month="' + ym.format('YYYY') + '-' + i + '"':'')+'>' + ym.format('YYYY') + '</div>'
                 + '<div'+(activeEvent?' data-goto-month="' + ym.format('YYYY') + '-' + i + '"':'')+'>' + moment(i, 'M').toDate().toLocaleString(opts.lang, { month: 'long' })+ '</div>'
                 + '<div'+(activeEvent?' data-goto-month="' + ym.format('YYYY') + '-' + i + '"':'')+'>' + ym.format('YYYY') + '</div>'
                 + '</div>';
        }
        html += '</div>';
    ...
    var updateDates = function(el, opts)
    {
    ...
    var checkDisabledDatesInRange = function(el, opts)
    {
    ...
    hms.util.Lightpick = function(options)
    {
        var self = this,
        ...
        self._onMouseDown = function(e)
        {
            ...
            if (target.classList.contains('lightpick__day') && target.classList.contains('is-available')) {
                ...
                if (target.hasAttribute('data-view-mode') && !target.classList.contains('disable_btn')) {
            ...
            if ((opts.startDate && !opts.endDate) || opts.repick) {
                ...
                if (opts.hoveringTooltip) {
                    ...
                    if (!opts.tooltipNights) {
                        ...
                        opts.locale.tooltip =  ['night', 'nights'];
                    }
    ...
    hms.util.Lightpick.prototype = {
        ...
        getNights:function(){
            return this._opts.selNights
        }

    };
    // 將hms, hms.util, hms.util.Lightpick暴露為全域變數
    if ( typeof noGlobal === strundefined ) {
        window.hms = hms || {};
        window.hms.util = hms.util || {};
        window.hms.util.Lightpick = hms.util.Lightpick;             
    }
    return hms.util.Lightpick;
}));
